...
public class RpmBuildCommand extends FedoraPackagerCommand<RpmBuildResult> {
	...
	public RpmBuildResult call(IProgressMonitor monitor)
		...
		monitor.subTask(NLS.bind(RpmText.RpmBuildCommand_callRpmBuildMsg,
				this.projectRoot.getSpecFile().getName()));
	...
	/**
	 * Do some extra initialization. Set build command and required defines for
	 * this Fedora project root.
	 */
	@Override
	...
	private String[] getBuildCommandList() throws RpmBuildCommandException {
		...
		fullRpmBuildCommand.add(this.projectRoot.getSpecFile().getLocation()
				.toOSString());
	...
	private class RpmBuildThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				result.setSuccessful(child.waitFor() == 0);
